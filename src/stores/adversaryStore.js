    ...
    async saveSelectedAdversary($api) {
      ...
      try {
        ...
        if (response.status !== 200) {
          throw new Error(`Non-200 HTTP response code from /api/v2/adversaries/${reqBody.adversary_id}: ${response.status}`);
        }
    ...
    generateSelectedAdversaryAbilities(adversary, abilitiesStore) {
      return (adversary.atomic_ordering || [])
        .map((step, index) => {
          ...
          if (!abilityTemplate) {
            console.warn(
              `[Automation] Ability not found in abilityStore for step ${index}: ${stepId}`
            );
            return null;
          }
          ...
          // Rebuild executors with correct facts
