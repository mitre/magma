...
function getChartData() {
    ...
    return [
        { 
            name: 'Alive (trusted)', 
            value: agents.value.filter((agent) => getAgentStatus(agent) === 'alive' && agent.trusted).length,
            itemStyle: { color: '#4a9' } 
        },
        { 
        ...
        { 
            name: 'Pending kill', 
            value: agents.value.filter((agent) => getAgentStatus(agent) === 'pending kill').length,
            itemStyle: { color: 'hsl(207deg, 61%, 53%)' } 
        },
        { 
            name: 'Dead', 
            value: agents.value.filter((agent) => getAgentStatus(agent) === 'dead').length,
            itemStyle: { color: '#c31' } 
        }
    ]
}
