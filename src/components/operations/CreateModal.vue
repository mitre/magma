...
function buildOperationPayload() {
  ...
  return {
    ...
    source: selectedSource.value?.id ? { id: sanitizeInput(selectedSource.value.id) } : undefined,
    planner: selectedPlanner.value?.id ? { id: sanitizeInput(selectedPlanner.value.id) } : undefined,
    adversary: { adversary_id: sanitizeInput(adversaryId) },
